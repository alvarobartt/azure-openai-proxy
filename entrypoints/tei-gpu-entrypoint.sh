...
export HF_HUB_USER_AGENT_ORIGIN="azure:foundry:gpu-cuda:inference:tei"

compute_cap=$(nvidia-smi --query-gpu=compute_cap --format=csv | sed -n '2p' | sed 's/\.//g')
if [ ${compute_cap} -eq 75 ]
then
    exec text-embeddings-router-75 --hostname "$UPSTREAM_HOST" --port "$UPSTREAM_PORT" "$@" &
elif [ ${compute_cap} -ge 80 -a ${compute_cap} -lt 90 ]
then
    exec text-embeddings-router-80 --hostname "$UPSTREAM_HOST" --port "$UPSTREAM_PORT" "$@" &
elif [ ${compute_cap} -eq 90 ]
then
    exec text-embeddings-router-90 --hostname "$UPSTREAM_HOST" --port "$UPSTREAM_PORT" "$@" &
else
    echo "ERROR: CUDA compute cap ${compute_cap} is not supported";
    exit 1
fi
UPSTREAM_PID=$!
