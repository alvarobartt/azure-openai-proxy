...
PROXY_PORT="${PORT:-80}"
# We need to unset the HOST and the PORT because otherwise it's picked up by TGI
unset HOST PORT
...
if [[ -n "$UPSTREAM_PORT" ]]; then
    if [[ "$UPSTREAM_PORT" -eq 80 ]]; then
        echo "ERROR: UPSTREAM_PORT environment variable cannot be set to 80 when running text-generation-inference with Docker, as the port 80 is reserved for the azure-openai-proxy service. Use another port as e.g. 8080, since 80 is the default port for TGI, but that leads to conflicts in this scenario."
        exit 1
    fi
...
azure-openai-proxy --host "$PROXY_HOST" --port "$PROXY_PORT" --upstream-host "$UPSTREAM_HOST" --upstream-port "$UPSTREAM_PORT" --upstream-type chat-completions &
PROXY_PID=$!
